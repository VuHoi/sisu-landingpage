...
type CustomBaseQueryFn = BaseQueryFn<RequestOptions, DataResponse, ErrorResponse>;
...
const axiosBaseQuery = ({ baseUrl }: { baseUrl: string } = { baseUrl: '' }): CustomBaseQueryFn => {
...
export const fetchAPI = async ({ url, method, headers, body, params }: RequestOptions) => {
  try {
    // Detect access token
    const accessToken = getToken('access_token');
    // Request
